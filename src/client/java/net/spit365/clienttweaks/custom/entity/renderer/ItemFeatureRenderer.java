...
public class ItemFeatureRenderer extends FeatureRenderer<PlayerEntityRenderState, PlayerEntityModel> {
    ...
    public void render(MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, PlayerEntityRenderState state, float limbAngle, float limbDistance) {
        Arrays.stream(CosmeticManager.loadedCustomCosmetics).filter(jsonObject -> ConfigManager.file().containsKey((String) jsonObject.get("name"))).forEach(jsonObject -> {
            JSONArray matrixOperations = (JSONArray) jsonObject.get("matrix_operations");
